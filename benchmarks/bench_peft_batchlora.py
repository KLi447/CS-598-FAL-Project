...
from mlora.profiler.profiler import (
...
# lora test number
parser.add_argument("--lora_cnt", type=int, default=4, help="The number of lora")
# test configure
parser.add_argument("--warmup", type=int, default=10, help="The step of warm up")
parser.add_argument("--repete", type=int, default=10, help="Total test iteration")
parser.add_argument(
    "--seq_len", type=int, default=128, help="The length of the sequence"
)
parser.add_argument(
    "--batch_size", type=int, default=8, help="The batch size of each lora input"
)
parser.add_argument(
    "--peft_mode",
    type=str,
...
def setup_llm_model() -> LlamaForCausalLM:
    ...
    additional_load_args = {"device_map": args.device, "torch_dtype": torch.float32}
    ...
    if load_bits is not None:
        ...
        qlora_4bit_compute_dtype = (
            torch.float16 if qlora_4bit_fp16 else qlora_4bit_compute_dtype
        )
        qlora_4bit_compute_dtype = (
            torch.bfloat16 if qlora_4bit_bf16 else qlora_4bit_compute_dtype
        )
...
def setup_labels() -> torch.Tensor:
    ...
    for _ in range(0, args.batch_size):
        batch_input_ids.append([random.randint(1, 10000) for _ in range(args.seq_len)])
    ...
    def lora_switch():
        for _ in range(0, args.repete):
            for lora_idx in range(0, args.lora_cnt):
                ...
                print(
                    f"total time : {time_cnt} throughtput: {lables.numel()/time_cnt} tokens/s"
                )
